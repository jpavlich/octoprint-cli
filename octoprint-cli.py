...
parser = argparse.ArgumentParser(
    prog="octoprint-cli",
    description="Command line tool for controlling OctoPrint 3D printer servers",
...
def continuous(args):
    try:
        while True:
            ...
            if not (
                caller.getState()
                in ("Operational", "Printing", "Paused", "Pausing", "Cancelling")
            ):
                print(colored(caller.getState(), "red", attrs=["bold"]))
                data = caller.get("/api/connection")
                print(
                    colored("Printer Profile: ", attrs=["bold"])
                    + data["options"]["printerProfiles"][0]["name"]
                )
                print(colored("Port: ", attrs=["bold"]) + str(data["current"]["port"]))
                print(
                    colored("Baudrate: ", attrs=["bold"])
                    + str(data["current"]["baudrate"])
                )
                lines = 4
            if caller.getState() == "Operational":
                ...
                if data["job"]["file"]["name"]:
                    print(
                        colored("Loaded File: ", attrs=["bold"])
                        + data["job"]["file"]["name"]
                    )
                    print(
                        colored("Estimated Print Time: ", attrs=["bold"])
                        + caller.getTotalTime()
                    )
                    print()
                    lines += 3
                ...
                print(
                    colored("Extruder Temp: ", attrs=["bold"])
                    + str(data2["temperature"]["tool0"]["actual"])
                    + "°C"
                )
                print(
                    colored("Extruder Target: ", attrs=["bold"])
                    + str(data2["temperature"]["tool0"]["target"])
                    + "°C"
                )
                print(
                    colored("Bed Temp: ", attrs=["bold"])
                    + str(data2["temperature"]["bed"]["actual"])
                    + "°C"
                )
                print(
                    colored("Bed Target: ", attrs=["bold"])
                    + str(data2["temperature"]["bed"]["target"])
                    + "°C"
                )
                lines += 6
            if caller.getState() == "Printing":
                ...
                print(
                    ...
                    + str(round(data["progress"]["completion"], 2))
                    + "%"
                )
                print(
                    colored("Estimated Print Time: ", attrs=["bold"])
                ...
                print(
                    colored("Print Time Left: ", attrs=["bold"])
                    + str(
                        datetime.timedelta(seconds=data["progress"]["printTimeLeft"])
                    ).split(".")[0]
                )
                print()
                print(colored("Temperature Status", attrs=["bold", "underline"]))
                ...
                print(
                    "Progress: |"
                    + ("#" * math.floor(data["progress"]["completion"] / 5))
                    + ("—" * math.ceil((100 - data["progress"]["completion"]) / 5))
                    + "| "
                    + str(round(data["progress"]["completion"], 2))
                    + "% Complete"
                )
...
com_continuous = subparsers.add_parser(
    "continuous",
    description="get continuous refreshing status with temperature information and progress information",
...
def layers(args):
    ...
    if data == 404:
        print(
            colored(
                "The DisplayLayerProgress is not installed or enabled on the OctoPrint server",
                "red",
                attrs=["bold"],
            )
        )
        sys.exit(1)
    elif type(data) == dict and caller.getState() in (
        "Printing",
        "Paused",
        "Pausing",
        "Finishing",
        ...
        print(
            colored("Current Layer: ", "white", attrs=["bold"])
            + data["layer"]["current"]
            + "/"
            + data["layer"]["total"]
        )
        print(
            colored("Current Height: ", "white", attrs=["bold"])
            + data["height"]["current"]
            + "/"
            + data["height"]["totalFormatted"]
            + "mm"
        )
        print(
            colored("Average Layer Duration: ", "white", attrs=["bold"])
            + data["layer"]["averageLayerDuration"]
            .replace("h", "")
            .replace("s", "")
            .replace("m", "")
        )
        print(
            colored("Last Layer Duration: ", "white", attrs=["bold"])
            + data["layer"]["lastLayerDuration"]
            .replace("h", "")
            .replace("s", "")
            .replace("m", "")
        )
...
com_layers = subparsers.add_parser(
    "layers",
    description="get layer information from the DisplayLayerProgress plugin",
...
def print_status(args):
    ...
    if state == "Offline":  # printer disconnected
        ...
        if state == "Operational" and selectedFile:
            print(colored("Printer Operational", "green", attrs=["bold"]))
            print(
                colored("Loaded File: ", attrs=["bold"]) + data["job"]["file"]["name"]
            )
            print(
                colored("Estimated Print Time: ", attrs=["bold"])
                + caller.getTotalTime()
            ...
            print(
                colored("Estimated Total Print Time: ", attrs=["bold"])
                + caller.getTotalTime()
            )
            print(
                colored("Estimated Print Time Left: ", attrs=["bold"])
                + caller.getTimeLeft()
            )
            print(
                colored("Progress: ", attrs=["bold"])
                + str(round(data["progress"]["completion"]))
                + "%"
            )
            print(
                colored("Extruder Temp: ", attrs=["bold"])
...
def print_i3status(args):
    state = caller.getState()
    if not state.startswith("Offline") and not state.startswith("Error"):
        job = caller.get("/api/job")["job"]
        temperature = caller.get("/api/printer")["temperature"]
        filename = Path(job["file"]["name"]).stem
        sys.stdout.write(filename + " | ")
        sys.stdout.write("Ext: " + str(temperature["tool0"]["actual"]) + "°C | ")
        sys.stdout.write("Bed: " + str(temperature["bed"]["actual"]) + "°C")


com_print_i3status = coms_print.add_parser(
    "i3status",
    description="view print job in i3status format ",
    help="view print job in i3status format",
)
com_print_i3status.set_defaults(func=print_i3status)
...
com_print_select = coms_print.add_parser(
    "select",
    description="select file on server to load",
...
def print_start(args):
    ...
    if code == 409 and caller.getState() in (
        "Printing",
        "Paused",
        "Pausing",
        "Cancelling",
...
com_print_start = coms_print.add_parser(
    "start",
    description="start print job on selected file",
...
com_connection = subparsers.add_parser(
    "connection",
    description="printer connection commands",
...
def connection_status(args):
    ...
    if data["current"]["state"] in ("Printing", "Operational"):
        ...
        print(
            colored("Baudrates: ", attrs=["bold"])
            + ", ".join(list(map(str, data["options"]["baudrates"])))
        )
...
com_temp = subparsers.add_parser(
    "temp",
    description="printer temperature commands",
...
def temp_status(args):
    ...
    if type(data) is dict:
        print(
            colored("Extruder Temp: ", attrs=["bold"])
            + str(data["temperature"]["tool0"]["actual"])
            + "°C"
        )
        print(
            colored("Extruder Target: ", attrs=["bold"])
            + str(data["temperature"]["tool0"]["target"])
            + "°C"
        )
        print(
            colored("Bed Temp: ", attrs=["bold"])
            + str(data["temperature"]["bed"]["actual"])
            + "°C"
        )
        print(
            colored("Bed Target: ", attrs=["bold"])
            + str(data["temperature"]["bed"]["target"])
            + "°C"
        )
...
com_temp_status = coms_temp.add_parser(
    "status",
    description="view temperature status of extruder and print bed",
...
def temp_extruder(args):
    try:
        if args.temperature > int(config["print"]["MaxExtruderTemp"]):
            print(
                colored(
                    "Target temp is higher than the limit set in the configuration file",
                    "red",
                    attrs=["bold"],
                )
            )
    ...
    code = caller.post(
        "/api/printer/tool",
        {"command": "target", "targets": {"tool0": args.temperature}},
    )
    if code == 204:
        print(
            colored(
                "Extruder temp set to " + str(args.temperature) + "°C",
                "green",
                attrs=["bold"],
            )
        )
...
com_temp_extruder = coms_temp.add_parser(
    "extruder",
    description="set extruder target temperature",
...
def temp_bed(args):
    try:
        ...
        pass
    code = caller.post(
        "/api/printer/bed", {"command": "target", "target": args.temperature}
    )
    if code == 204:
        print(
            colored(
                "Bed temp set to " + str(args.temperature) + "°C",
                "green",
                attrs=["bold"],
            )
        )
...
def system_restart(args):
    prompt = input(
        colored(
            "You are restarting the server. Are you sure you wish to continue?",
            attrs=["bold"],
        )
        + " [Y/n]: "
    )
...
def system_restart_safe(args):
    prompt = input(
        colored(
            "You are restarting the server to safe mode. Are you sure you wish to continue?",
            attrs=["bold"],
        )
        + " [Y/n]: "
    )
...
com_system_restart_safe = coms_system.add_parser(
    "restart-safe",
    description="restart OctoPrint server to safe mode",
...
def system_reboot(args):
    prompt = input(
        colored(
            "You are rebooting the server. Are you sure you wish to continue?",
            attrs=["bold"],
        )
        + " [Y/n]: "
    )
...
def system_shutdown(args):
    prompt = input(
        colored(
            "You are shutting down the server. Are you sure you wish to continue?",
            attrs=["bold"],
        )
        + " [Y/n]: "
    )
...
com_system_shutdown = coms_system.add_parser(
    "shutdown",
    description="shutdown OctoPrint server",
...
com_files = subparsers.add_parser(
    "files",
    description="server file management commands",
...
def files_list(args):
    ...
    print(
        ...
        + (" " * longestName)
        + colored("TYPE", attrs=["bold"])
        + (" " * longestType)
        + colored("SIZE", attrs=["bold"])
    )  # table headings
    longestName += 4
    longestType += 4
    ...
    if container == "files":
        print(
            colored("\nFree space: ", attrs=["bold"])
            + str(round(data["free"] / 1073741824.0, 3))
            + " GB"
        )  # disk space
    sys.exit(0)
...
def files_info(args):
    ...
    print(
        ...
        + data["refs"]["resource"].replace(destination + "/api/files/local/", "")
    )
    print(colored("Type: ", attrs=["bold"]) + data["type"])
    try:
        print(
            colored("Size: ", attrs=["bold"])
            + str(round(data["size"] / 1048576.0, 2))
            + " MB"
        )
    except KeyError:
        pass
    if data["type"] == "machinecode":
        try:
            print(
                ...
                + str(
                    datetime.timedelta(
                        seconds=data["gcodeAnalysis"]["estimatedPrintTime"]
                    )
                ).split(".")[0]
            )
        except KeyError:
        ...
        try:
            print(
                colored("Successful Prints: ", attrs=["bold"])
                + str(data["prints"]["success"])
            )
        except KeyError:
            pass
        try:
            print(
                colored("Failed Prints: ", attrs=["bold"])
                + str(data["prints"]["failure"])
            )
        except KeyError:
            pass
    sys.exit(0)
...
com_files_info = coms_files.add_parser(
    "info",
    description="get info on a file or folder",
...
def files_upload(args):
    ...
    if data == 409:
        print(
            colored(
                "File upload is in conflict with server state", "red", attrs=["bold"]
            )
        )
        sys.exit(1)
    if type(data) is dict:
